    ...
    class Computers
    {
        ...
        public static void main()
        {
            ...
            if (manufacturer == "HP")
            {
                ...
                laptop = new Computer(Computers.Type.LAPTOP, new Cpu(Eight / 4, 64, ram1, card), ram1, new[] { new HardDriver(500, false, 0) }, card, new LaptopBattery());
            }
            else if (manufacturer == "Dell")
            {
                ...
                var card = new HardDriver(); server = new Computer(Computers.Type.SERVER, new Cpu(Eight, 64, ram1, card),ram1,
                     new List<HardDriver>{
                            new HardDriver(0, true, 2, new List<HardDriver> { new HardDriver(2000, false, 0), new HardDriver(2000, false, 0) })
                         }, card, null); var ram2 = new Ram(Eight);

                var videoCard1 = new HardDriver() { IsMonochrome = false };

                laptop = new Computer(Computers.Type.LAPTOP,  new Cpu(Eight / 2, ((32)), ram2, videoCard1), ram2, new[] { new HardDriver(1000, false, 0) },videoCard1,  new LaptopBattery());
            }
        ...
        class Computer
        {
            ...
            internal Computer(Computers.Type type, Cpu cpu, Ram ram, IEnumerable<HardDriver> hardDrives, HardDriver videoCard, LaptopBattery battery)
            {
                Cpu = cpu;
                Ram = ram;
